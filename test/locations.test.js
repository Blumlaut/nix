...
test('the heatmap only publishes cells backed by several nixers', () => {
  ...
  // Two nixes from one nixer in a second cell: too thin to publish.
  ...
  assert.equal(map.minCellNixers, 3, 'the floor travels with the response');
  assert.equal(map.cells.some((c) => c.lat === 50.94), false, 'a lone nixer is not a heat cell');
});
...
test('the heatmap can be filtered by nixer and by range', () => {
  ...
  assert.equal(locations.heatmap({ range: 'all' }).cells.length, 1);
...
test('POST /api/nix records the fix and the heatmap serves cells only', async () => {
  ...
  assert.deepEqual(map.cells.map((c) => [c.lat, c.lon]), [[52.52, 13.4]]);
