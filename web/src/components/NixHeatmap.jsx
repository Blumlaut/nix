...
export default function NixHeatmap({ cells, cellDegrees = 0.01 }) {
  ...
  useEffect(() => {
    ...
    // part of the office warmer without blurring the cell boundaries.
    for (const cell of normalized) {
      L.circle([cell.lat, cell.lon], {
        radius: cellDegrees * METRES_PER_DEGREE * 0.8,
        stroke: false,
    ...
    for (const cell of normalized) {
      ...
      rect.bindTooltip(
        `${cell.n} nix${cell.n === 1 ? '' : 'es'} · ${cell.nixers} nixer${cell.nixers === 1 ? '' : 's'}`,
      );
      rect.addTo(group);
